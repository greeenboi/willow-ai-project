    ...
    def get_contextual_response(self, user_message: str, session_context: Dict, lead_info: Dict) -> Dict:
        ...
        if demo_shown and self.should_offer_meeting(user_message, lead_info):
            return {
                ...
                "updated_lead_info": lead_info,
                "start_meeting_booking": True  # Flag to start meeting booking mode
            }

        # If user is trying to end the conversation after demo shown, push for meeting
        if demo_shown and any(keyword in user_message.lower() for keyword in ["that's all", "end the call", "we can end", "goodbye", "bye", "thank you", "thanks"]):
            return {
                "meeting_offer": "Before we wrap up, I'd like to make sure you have a clear next step. Sounds like Willow AI could be a great fit for your team. I'd love to set up a quick demo with our sales expert to walk you through how companies like yours are using it.\n\nYou can book a meeting directly using our calendar system at {cal_link} or I can help you schedule it. Would you prefer to book it yourself or would you like me to guide you through the process?",
                "persona": persona,
                "intent": "meeting_offer",
                "updated_lead_info": lead_info,
                "start_meeting_booking": True  # Flag to start meeting booking mode
        ...
        # Only proceed with demo logic if demo hasn't been shown yet
        if not demo_shown:
            ...
            # Check if we should offer a demo (only if not already shown)
        ...
        # Check if we should offer a meeting with AE (fallback for cases not caught by earlier checks)
        # This handles cases where demo_shown is false but user still shows meeting interest
        if not demo_shown and self.should_offer_meeting(user_message, lead_info):
            return {
                "meeting_offer": "Sounds like Willow AI could be a great fit for your team. I'd love to set up a quick demo with our sales expert to walk you through how companies like yours are using it.\n\nYou can book a meeting directly using our calendar system at {cal_link} or I can help you schedule it. Would you prefer to book it yourself or would you like me to guide you through the process?",
                "persona": persona,
                "intent": "meeting_offer",
        ...
        # Final fallback for meeting readiness - only if not caught by earlier checks
        # and only if we haven't shown a demo yet (otherwise we'd use the demo_shown checks above)
        if not demo_shown and completion >= 75 and self.detect_meeting_readiness(user_message, lead_info):
            return {
                "meeting_offer": "Excellent! It sounds like Willow AI could be a great fit for your team. I'd love to set up a customized demo call with John, our account executive, who can walk you through exactly how this would work for your specific use case.\n\nYou can book a meeting directly using our calendar system at {cal_link} or I can help you schedule it. Would you prefer to book it yourself or would you like me to guide you through the process?",
                "persona": persona,
                "intent": "meeting_offer",
                "updated_lead_info": lead_info,
                "start_meeting_booking": True  # Flag to start meeting booking mode
...
"""
    ...
    def should_offer_meeting(self, user_message: str, lead_info: Dict) -> bool:
        ...
        # If demo has been shown, be more aggressive in pushing for a meeting
        if demo_shown:
            # Check if user is trying to end the conversation
            conversation_ending_indicators = [
                "that's all", "end the call", "we can end", "goodbye", "bye", 
                "that's it", "no more questions", "nothing else", "that's enough",
                "thank you", "thanks", "appreciate it", "got it", "understood",
                "that's helpful", "that's useful", "that's good to know"
            ]

            # If user is trying to end conversation after demo, push for meeting
            if any(indicator in text_lower for indicator in conversation_ending_indicators):
                return True

            # Positive response indicators (broader to catch more cases)
            positive_response_indicators = [
                "yes", "yeah", "sure", "okay", "ok", "good", "great", "perfect",
                "sounds good", "makes sense", "interesting", "helpful", "useful",
                "valuable", "beneficial", "worth", "nice", "impressive", "cool",
                "awesome", "excellent", "fantastic", "wonderful", "amazing",
                "i like", "i love", "i think", "i believe", "i feel", "i'd like",
                "i would", "we could", "we should", "we might", "we can"
            ]

            # If user gives any positive response after demo, push for meeting
            if any(indicator in text_lower for indicator in positive_response_indicators) and not any(neg in text_lower for neg in ["no", "not", "don't", "won't", "can't"]):
                return True

        # Meeting interest indicators - expanded for better detection
        meeting_interest_keywords = [
            ...
            "yes", "sure", "definitely", "absolutely", "of course", "certainly",
            "that makes sense", "sounds right", "agree", "positive", "helpful",
            "useful", "valuable", "beneficial", "worth it", "good solution"
        ]
        ...
        demo_completion_keywords = [
            ...
            "i saw", "i watched", "i viewed", "i liked", "i enjoyed", "i appreciate"
        ]

        # Check if user showed positive response to demo content or general interest
        ...
        # Check for direct meeting requests
        direct_meeting_requests = [
            "schedule", "book", "meeting", "call", "talk to", "speak with", 
            "connect me", "set up", "appointment", "when can we", "available"
        ]
        has_direct_meeting_request = any(keyword in text_lower for keyword in direct_meeting_requests)

        # Original conditions for offering a meeting (now as fallback)
        return (completion > 75 and (
            (has_demo_interest and demo_shown) or
            has_direct_meeting_request or
            (has_meeting_interest and demo_shown) or
            # If they respond positively to "is this a good fit" question
            (len(text_lower.split()) <= 5 and 
             any(pos in text_lower for pos in ["yes", "yeah", "sure", "good", "great", "perfect"]) and
             not any(neg in text_lower for neg in ["no", "not", "don't", "won't", "can't"]))
        ))
    ...
    def should_show_media(self, user_message: str, session_context: Dict) -> Optional[Dict]:
        ...
        # Check if demo has already been shown using lead_info from session_context
        lead_info = session_context.get("lead_info", {})
        demo_already_shown = lead_info.get("demo_shown", False)

        # If demo has already been shown, don't show it again
        if demo_already_shown:
            # Instead, check if we should show other media types
            media_triggers = {
        ...
        demo_interest_keywords = [
            "yes", "sure", "okay", "alright", "sounds good", "let's do it", "show me",
            "i'd like to see", "please do", "go ahead", "definitely", "absolutely",
            "of course", "certainly", "please show", "would love to", "interested",
            "show it", "that would be great", "good idea", "why not", "proceed",
            "let me see", "show the demo", "yes please", "yeah", "yep", "yup", "ok"
        ]
        agent_asked_demo = session_context.get("agent_asked_demo", False)

        # More robust check for demo interest - check if any keyword is in the message
        # or if the message is a short positive response (like "yes", "sure", etc.)
        if agent_asked_demo and (
            any(keyword in text_lower for keyword in demo_interest_keywords) or
            (len(text_lower.split()) <= 3 and not any(neg in text_lower for neg in ["no", "not", "don't", "later"]))
        ):
    ...
    def format_agent_response(self, response_data: Dict, user_message: str) -> str:
        ...
        # Handle meeting offer
        if response_data.get("meeting_offer"):
            # Replace {cal_link} placeholder with actual cal.com link
            meeting_offer = response_data["meeting_offer"]
            # The actual cal.com link will be inserted in main.py
            return meeting_offer

        # Handle direct booking response (when user wants to book directly via cal.com)
        if response_data.get("direct_booking_response"):
            return response_data["direct_booking_response"]

        # Handle guided booking response (when agent is guiding user through booking process)
        if response_data.get("guided_booking_response"):
            return response_data["guided_booking_response"]
